...
public class Calculator extends VBox implements EventHandler<ActionEvent>{
	...
	public void handle(ActionEvent event) {
		...
		if (value.equals("=")) {
			String[] lista = displayText.getText().split("(?<=(\\+|-|\\*|/))" 
															+ "|" 
															+ "(?=(\\+|-|\\*|/))");
			number1 = lista[0];
			number2 = lista[2];
			...
			float number1Int = Float.valueOf(number1);
			float number2Int = Float.valueOf(number2);
			
			System.out.println(operator);
			
			switch (operator) {
				case "+":
					displayText.setText(Float.toString(number1Int + number2Int));
					break;
				case "-":
					displayText.setText(Float.toString(number1Int - number2Int));
					break;
